...
const char* message_type_to_string(MessageType messageType) {
    switch (messageType) {
        case CONNECTED_ACK: return "DEVICE_CONNECTED";
        case CONFIG: return "CONFIG";
        case ANALOG: return "ANALOG";
        case ANALOG_ACK: return "ANALOG_ACK";
        case PWM_SETUP: return "PWM";
  ...
  MessageType string_to_message_type(const char* messageType) {
    if (strcmp(messageType, "DEVICE_CONNECTED") == 0) return CONNECTED_ACK;
    ...
    if (strcmp(messageType, "PWM") == 0) return PWM_SETUP;
