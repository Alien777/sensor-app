...
import static pl.lasota.sensor.payload.PayloadType.ANALOG_READ_ONE_SHOT_RESPONSE;
import static pl.lasota.sensor.payload.PayloadType.CONNECTED_ACK;
...
class ManagerFlowTest {
    ...
    void listening_sensor_execute_test() throws Exception {
        ...
        JsonNode payloadTypeMock = Mockito.mock(JsonNode.class);
        Mockito.when(jn.findValue(Mockito.same("payloadType"))).thenReturn(payloadTypeMock);
        Mockito.when(payloadTypeMock.asText()).thenReturn(ANALOG_READ_ONE_SHOT_RESPONSE.name());
    ...
    void cron_node_execute_stop_test() throws Exception {
        ...
        JsonNode payloadTypeMock = Mockito.mock(JsonNode.class);
        Mockito.when(jn.findValue(Mockito.same("cron"))).thenReturn(payloadTypeMock);
        Mockito.when(payloadTypeMock.asText()).thenReturn("5");
...
//    //
    ...
    void js_condition_false_variable_test() throws Exception {
        ...
        JsonNode payloadTypeMock = Mockito.mock(JsonNode.class);
        Mockito.when(jn.findValue(Mockito.same("payloadType"))).thenReturn(payloadTypeMock);
        Mockito.when(payloadTypeMock.asText()).thenReturn(CONNECTED_ACK.name());
